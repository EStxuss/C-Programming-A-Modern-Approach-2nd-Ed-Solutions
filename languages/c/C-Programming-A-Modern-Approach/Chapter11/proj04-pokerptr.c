// modified poker.c that moves all global variables into main and communication
// happens by passing pointers as arguments
...
void analyze_hand(int hand[NUM_CARDS][2], bool *royalFlush, bool *straight,
                  bool *flush, bool *four, bool *three, int *pairs);
void print_result(bool royalFlush, bool straight, bool flush, bool four,
...
int main(void) {
    ...
    for (;;) {
        read_cards(hand);
        analyze_hand(hand, &royalFlush, &straight, &flush, &four, &three,
                     &pairs);
        print_result(royalFlush, straight, flush, four, three, pairs);
    }
}
...
void analyze_hand(int hand[NUM_CARDS][2], bool *royalFlush, bool *straight,
    ...
    *straight = true;
    ...
    if (!(*straight)) { // check low ace straight
        int i;
        bool twoToFive = true;
        ...
        if (twoToFive && hand[NUM_CARDS - 1][0] == 12)
            *straight = true;
    }
    ...
    *royalFlush = *straight && *flush && hand[0][0] == 8;
}
